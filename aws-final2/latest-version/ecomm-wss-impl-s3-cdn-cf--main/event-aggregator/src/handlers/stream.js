...
const { DynamoDBDocumentClient, UpdateCommand, BatchWriteCommand, ScanCommand } = require("@aws-sdk/lib-dynamodb");
...
const AGG_BUCKET = process.env.AGG_BUCKET;
const CONNECTIONS_TABLE = process.env.CONNECTIONS_TABLE;
const WEBSOCKET_ENDPOINT = process.env.WEBSOCKET_ENDPOINT;
const MAX_RETRIES = 3;

async function pushMetricsToWebSocket(aggregations) {
  if (!CONNECTIONS_TABLE || !WEBSOCKET_ENDPOINT) {
    return; // WebSocket push not configured
  }
  ...
  try {
    ...
    // Prepare metrics in same format as ws-push.js
    ...
    const metrics = parseMetrics(items.Items || []);
    ...
    console.log(`[WS-PUSH] Pushed metrics to ${connections.length} clients`);
  } catch (err) {
    console.error("[WS-PUSH] Error pushing metrics:", err.message);
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Push metrics to WebSocket clients (async, non-blocking)
    pushMetricsToWebSocket(aggregations).catch(err =>
      console.error("[STREAM] Failed to push metrics to WebSocket:", err.message)
    );
